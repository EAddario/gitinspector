...
FILTERING_EMAIL_INFO_TEXT = N_(
    "The authors with the following emails were excluded from the statistics due to the specified "
    "exclusion patterns")
...
class FilteringOutput(Outputable):
    ...
    def output_json(self):
        if has_filtered():
            ...
            output += FilteringOutput.__output_json_section__(_(FILTERING_AUTHOR_INFO_TEXT), __filters__["author"][1],
                                                              "authors")
            output += FilteringOutput.__output_json_section__(_(FILTERING_EMAIL_INFO_TEXT), __filters__["email"][1],
                                                              "emails")
            output += FilteringOutput.__output_json_section__(_(FILTERING_COMMIT_INFO_TEXT), __filters__["revision"][1],
                                                              "revision")
            output = output[:-1]
            output += "\n\t\t}"
            print(output, end="")
    ...
    def output_xml(self):
        if has_filtered():
            ...
            FilteringOutput.__output_xml_section__(_(FILTERING_COMMIT_INFO_TEXT), __filters__["revision"][1],
                                                   "revision")
            print("\t</filtering>")

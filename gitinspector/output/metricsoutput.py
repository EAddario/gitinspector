...
from ..metrics import (__metric_eloc__, METRIC_CYCLOMATIC_COMPLEXITY_THRESHOLD,
...
class MetricsOutput(Outputable):
    ...
    def output_text(self):
        ...
        if self.metrics.cyclomatic_complexity_density:
            ...
            for i in sorted(set([(j, i) for (i, j) in self.metrics.cyclomatic_complexity_density.items()]),
                            reverse=True):
    ...
    def output_html(self):
        ...
        if self.metrics.cyclomatic_complexity:
            ...
            for num, i in enumerate(
                    sorted(set([(j, i) for (i, j) in self.metrics.cyclomatic_complexity.items()]), reverse=True)):
        ...
        if self.metrics.cyclomatic_complexity_density:
            ...
            for num, i in enumerate(
                    sorted(set([(j, i) for (i, j) in self.metrics.cyclomatic_complexity_density.items()]),
                           reverse=True)):
